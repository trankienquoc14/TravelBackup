                ...
                <div class="stat-header-flex">
                    ...
                    <div class="stat-info">
                        ...
                        <h4><span id="checkedInPax"><?= $checkedInPax ?></span> / <span
                                id="totalPax"><?= $totalPax ?></span> khách</h4>
                    </div>
                </div>
                ...
                <form method="POST" action="guide.php?action=updateStatus" class="w-100">
                    ...
                    <div class="d-flex gap-2">
                        <select name="status" class="form-select status-select flex-grow-1">
                            ...
                            <option value="upcoming" <?= !$canUpcoming ? 'disabled' : '' ?>
                                <?= $departure['status'] == 'upcoming' ? 'selected' : '' ?>>
                                Chờ khởi hành
                            </option>

                            <option value="ongoing" <?= !$canOngoing ? 'disabled' : '' ?>
                                <?= $departure['status'] == 'ongoing' ? 'selected' : '' ?>>
                                Đang diễn ra
                            </option>

                            <option value="completed" <?= !$canCompleted ? 'disabled' : '' ?>
        ...
        <div class="col-xl-8 col-lg-7">
            <div class="ui-box">
                <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3"
                    style="border-style: dashed !important; border-color: var(--app-border) !important;">
                    ...
                    <div class="d-flex gap-2 flex-wrap justify-content-end">
                        ...
                        <button
                            onclick="exportTableToExcel('attendanceTable', 'Danh_Sach_Khach_<?= $departure['departure_id'] ?>')"
                            class="btn btn-outline-success btn-sm fw-bold" style="border-radius: 8px;">
                            <i class="bi bi-file-earmark-excel"></i> Xuất Excel
                        </button>
                        <button onclick="window.print()"
                            class="btn btn-outline-secondary btn-sm fw-bold d-none d-md-block"
                ...
                <div id="qr-reader-container" class="mb-4 d-none">
                    <div class="p-3 bg-light border rounded-3 text-center">
                        <h6 class="fw-bold text-primary mb-2"><i class="bi bi-camera-video me-1"></i> Camera Check-in
                        </h6>
                        <div id="qr-reader" style="width: 100%; max-width: 500px; margin: 0 auto;"></div>
                        <button id="btn-close-qr" class="btn btn-sm btn-outline-danger mt-3 px-4 rounded-pill">Đóng
                            Camera</button>
                    </div>
                                        ...
                                        <td class="text-center action-column">
                                            ...
                                            <div class="d-flex justify-content-center gap-2 align-items-center">
                                                ...
                                                <?php if (empty($b['checkin_id'])): ?>
                                                    ...
                                                    <form method="POST" action="guide.php?action=checkin" class="m-0 form-checkin">
                                                        ...
                                                        <button type="submit" class="btn btn-success btn-action-table px-2">
                ...
                <div class="modal-content"
                    ...
                    <div class="modal-header border-bottom-0 pb-0">
                        <h5 class="modal-title fw-bold text-dark"><i
                                class="bi bi-person-vcard text-primary me-2"></i>Thông tin hành khách</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="p-3 bg-light rounded-3 mb-3 border">
                            <h6 class="fw-bold text-primary mb-1 fs-5" id="detail-name">...</h6>
                            <p class="text-muted small mb-0"><i class="bi bi-telephone-fill me-1"></i> <span
                                    id="detail-phone">...</span></p>
                        </div>
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item px-0 d-flex flex-column align-items-start border-bottom-0">
                                ...
                                <p class="mb-0 bg-light p-2 rounded-2 w-100"
                                    style="font-size: 0.9rem; border: 1px dashed #e2e8f0;" id="detail-note">...</p>
                            </li>
                        </ul>
                    </div>
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        // =========================
        // CODE PUSHER CŨ CỦA BẠN GIỮ NGUYÊN BÊN DƯỚI NÀY
        ...
        pageSocket.on("connect", () => {
            console.log("🟢 Trang Chi tiết Đoàn đã kết nối Socket!");
        });

        pageSocket.on("system_notification", function (data) {
            console.log("👉 Phát hiện biến động đơn hàng mới:", data);

            let typeName = data.type || '';
            let messageStr = data.message || '';

            // Nhận diện nếu đây là sự kiện Đặt tour mới (có thể có chữ "Đơn Hàng", "Xác Nhận", "Thanh Toán" hoặc "Hủy")
            let isBookingEvent = typeName === 'Đơn Hàng' || typeName === 'Xác Nhận' || typeName === 'Thanh Toán' || typeName === 'Hủy Đơn' || messageStr.toLowerCase().includes('đặt đơn mới');

            if (isBookingEvent) {
                console.log("🔄 Bắt đầu tải ngầm danh sách khách hàng mới nhất...");

                // Dùng AJAX tải lại chính đường dẫn URL hiện tại
                fetch(window.location.href)
                    ...
                    .then(html => {
                        ...
                        // Cập nhật lại BẢNG DANH SÁCH KHÁCH HÀNG (table)
                        let newTable = doc.getElementById('attendanceTable');
                        let oldTable = document.getElementById('attendanceTable');

                        if (newTable && oldTable) {
                            oldTable.innerHTML = newTable.innerHTML;
                            console.log("✅ Đã cập nhật ngầm Bảng danh sách hành khách!");
                        }

                        // Cập nhật lại KHỐI TỔNG QUAN (Tiến độ check-in, tổng pax)
                        let newStats = doc.querySelector('.top-stats-container');
                        let oldStats = document.querySelector('.top-stats-container');

                        if (newStats && oldStats) {
                            oldStats.innerHTML = newStats.innerHTML;
                            console.log("✅ Đã cập nhật ngầm Khối thống kê số lượng!");
                        }

                        // Khởi tạo lại hàm Tooltip Bootstrap (đề phòng các nút mới chèn vào bị mất hiệu ứng)
                    ...
                    .catch(error => console.error("Lỗi khi tải ngầm danh sách:", error));
            }
        });
